...
int hook_id;//variavel global para puder ser acessivel
...
int (timer_subscribe_int)(uint8_t *bit_no) {
  ...
  hook_id = 0; //hook_id = TIMER0_IRQ

  if(sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &hook_id) != OK){
    printf("Erro em sys_irqsetpolicy na funcao: 'timer_subscribe_int'\n");
    if(sys_irqenable(&hook_id) != OK){//se a set policy der erro
...
int (timer_unsubscribe_int)() {
  if(sys_irqdisable (&hook_id) !=OK){//disables interrupts
    printf("Erro em sys_irqdisable na funcao: 'timer_unsubscribe_int'\n");
    return FUNCTION_FAIL;
  }
  if(sys_irqrmpolicy(&hook_id) != OK){//unsubscribes previous subscription
